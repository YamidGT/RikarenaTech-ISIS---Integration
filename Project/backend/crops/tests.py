...
class CropsAPITest(APITestCase):
	...
	def test_create_list_retrieve_update_delete_flow(self):
		...
		# handle pagination
		results = list_data.get("results", list_data if isinstance(list_data, list) else [])
		...
		# Update (patch)
		patch_resp = self.client.patch(f"/api/crops/{crop_id}/", {"area": 3.5}, format="json")
		...
		self.assertIn(forbidden_resp.status_code, (status.HTTP_404_NOT_FOUND, status.HTTP_403_FORBIDDEN))

		# Non-owner delete should fail
		del_resp = self.client.delete(f"/api/crops/{crop_id}/")
		self.assertIn(del_resp.status_code, (status.HTTP_404_NOT_FOUND, status.HTTP_403_FORBIDDEN))

		# Owner can delete
		self.client.force_authenticate(user=self.user1)
		owner_del = self.client.delete(f"/api/crops/{crop_id}/")
		self.assertIn(owner_del.status_code, (status.HTTP_204_NO_CONTENT, status.HTTP_200_OK, status.HTTP_202_ACCEPTED))
