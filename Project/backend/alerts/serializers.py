...
class AlertWriteSerrializer(serializers.ModelSerializer):
    ...
    def validate(self, data): # type: ignore
        ...
        if scope == "municipal" and not data.get('municipality'):
            raise serializers.ValidationError("Municipality must be set for municipal scope alerts.")

        elif scope == "departmental" and not data.get('department'):
            raise serializers.ValidationError("Department must be set for departmental scope alerts.")
    ...
    def create(self, validated_data):
        ...
        images = request.FILES.getlist('images') or request.FILES.getlist('images[]') if request else []
        
        # Validate minimum and maximum images
        min_images = 2
        max_images = getattr(settings, 'MAX_IMAGES_PER_ALERT', settings.MAX_IMAGES_PER_POST)
    ...
    def to_representation(self, instance):
        ...
        data['images'] = AlertImageSerializer(instance.images.all(), many=True, context=self.context).data
        return data
